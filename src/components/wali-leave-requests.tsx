...
export default function WaliLeaveRequests() {
  ...
  const teacherClassIds = classList.map((c) => c.id)
  ...
  const fetchLeaveRequests = useCallback(async () => {
    try {
      ...
      if (filterClass) params.set('class_id', filterClass)
        ...
        // Filter by first teacher class if no class selected
        // API only supports single class_id filter
      }
      const res = await fetch(`/api/leave-requests?${params.toString()}`, { headers: getAuthHeaders() })
      const json = await res.json()
      if (json.success) {
        // Filter to only show requests from teacher's classes
        const filtered = filterClass
          ? json.data
          : json.data.filter((r: LeaveRequestItem) =>
              teacherClassIds.includes(r.student?.class?.id)
            )
  ...
  }, [filterStatus, filterType, filterClass, teacherClassIds])
  ...
  const fetchClasses = useCallback(async () => {
    try {
      ...
      const json = await res.json()
      if (json.success) {
        // Filter classes that this teacher manages
        // For now, show all classes (in real app, filter by teacher assignment)
  ...
  const handleApprove = async (item: LeaveRequestItem) => {
    ...
    try {
      const res = await fetch('/api/leave-requests', {
        ...
        body: JSON.stringify({
          id: item.id,
          status: 'approved_wali_kelas',
          approved_by_wali_id: currentUser?.teacherId,
        }),
      })
      const json = await res.json()
      if (json.success) {
        toast.success('Izin/sakit berhasil disetujui (Level Wali Kelas)')
        setShowDetailDialog(false)
        // Send notification to admin
        // Note: In real app, send to admin users
        fetchLeaveRequests()
      } else {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm">Kelola pengajuan izin dan sakit siswa di kelas Anda</p>
      </div>
